 ...
 *
 * Top-up aware: the series is segmented at balance INCREASES (a top-up or
 * refill), because a raw first-vs-last delta across a refill is meaningless
 * (negative => "topped up" => burn gets thrown away entirely). Burn is
 * computed on the most recent segment that actually shows a decrease, so a
 * refill today doesn't erase the burn measured since it.
 */
export function computeProviderDailyBurn(entries) {
  const points = entries.filter((e) => e.balance != null);
  if (points.length < 2) return null;

  // Segment at upward jumps. A jump counts as a refill when the balance
  // increased by more than noise (>= $0.05 or > 5% of the prior value).
  const segments = [];
  let seg = [points[0]];
  for (let i = 1; i < points.length; i++) {
    const prev = points[i - 1].balance;
    const cur = points[i].balance;
    const jumpUp = cur > prev &&
      (cur - prev >= 0.05 || (prev > 0 && (cur - prev) / prev > 0.05));
    if (jumpUp) {
      segments.push(seg);
      seg = [points[i]];
    } else {
  ...
  // Walk segments newest -> oldest; first one with a real decrease wins.
  // (A segment with a net increase is a refill we already cut at, so it
  // cannot be the source of burn.)
  for (let s = segments.length - 1; s >= 0; s--) {
    const segPoints = segments[s];
    ...
    if (delta <= 0) continue; // flat or refill within segment — keep scanning
    ...
    return {
      ...
      points: segPoints.length,
      segmentStart: first.timestamp,
      segmentEnd: last.timestamp,
    };
  }

  // No usable segment: not enough time or everything was refills.
  return null;
}
...
export function buildBurnRate({
  ...
  return {
    ...
    notes: [
      ...
      'Providers on free monthly credits expose no balance endpoint, so their burn is not directly measurable.',
